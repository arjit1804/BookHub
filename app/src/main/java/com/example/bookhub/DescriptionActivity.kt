...
class DescriptionActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //now we arre using post request so we have to give the jsonrequest some value which we did not give while making get request
    ...
    class DBAsyncTask(val context: Context, val bookEntity: BookEntity, val mode:Int) : AsyncTask<Void, Void, Boolean>(){
        /*
        * mode 1 -> check in db that the book is in favourite or not
        * mode 2 -> save the book in db as favourites
        * mode 3 -> remove the book from db
        */
        ...
        override fun doInBackground(vararg p0: Void?): Boolean {
            when(mode){
                1 -> {
                    //check in db that the book is in favourite or not
